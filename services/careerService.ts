...
export const createCareer = async (careerData: Partial<Career>): Promise<Career> => {
  ...
  const response = await axios.post(`${BACKEND_URL}/${carreraPath}/create`, careerData, {
    headers: getAuthHeaders(),
  });
  return response.data;
};
...
export const updateCareer = async (id: string, careerData: Partial<Career>): Promise<Career> => {
  const response = await axios.put(`${BACKEND_URL}/${carreraPath}/${id}`, careerData, {
    headers: getAuthHeaders(),
  });
  return response.data;
};
...
export const toggleCareerStatus = async (id: string, enable: boolean): Promise<Career> => {
